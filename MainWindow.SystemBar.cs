    ...
    // Cập nhật: 2026-03-14 - Fix đổi segment theo Gemini: Cancel workers -> re-queue chunks -> restart với numConnections mới
    // =======================================================================
    using System;
                    ...
                    try
                    {
                        ...
                        using (HttpClient client = new HttpClient(new HttpClientHandler { AllowAutoRedirect = false, MaxConnectionsPerServer = 16 }))
                        {
                            ...
                            try
                            {
                                ...
                                while (!chunkQueue.IsEmpty || _remainingRanges.Count > 0)
                                {
                                    ...
                                    // Nếu có lệnh đổi segment, chỉ cần reset flag
                                    // Workers sẽ tự động restart với numConnections mới ở vòng lặp tiếp theo
                                    if (_isReSegmenting)
                                    {
                                        _isReSegmenting = false;
                                        lastNumConnectionsForUI = -1; // Force UI reset ở vòng lặp sau
                                        UpdateStatus($"Đang tải với {numConnections} luồng...", "Cyan");
                                    }
                                    ...
                                    // Re-queue các range chưa hoàn thành vào queue
                                    lock (lockObj)
                                    {
                                        if (_remainingRanges.Count > 0)
                                        {
                                            // Nếu không có chunk nào được xử lý trong vòng này mà vẫn còn remaining ranges
                                            // thì có nghĩa là lỗi kết nối liên tục - hủy quá trình
                                    ...
                                    // Chỉ break khi file đã tải xong VÀ queue đã cạn
                                    if (totalDownloaded >= fileSize && chunkQueue.IsEmpty && _remainingRanges.Count == 0) break;
                                }
                            }
                            finally
                            {
            ...
            private void CboSegmentCount_SelectionChanged(object sender, SelectionChangedEventArgs e)
            {
                ...
                if (CboSegmentCount?.SelectedItem is ComboBoxItem item && item.Content != null)
                {
                    if (int.TryParse(item.Content.ToString(), out int newCount))
                    {
                        ...
                        UpdateStatus($"Đang cấu hình lại sang {newCount} luồng. Vui lòng đợi...", "Cyan");

                        // CỐT LÕI: Hủy Token hiện tại để ép các Worker quăng Exception và thoát ngay lập tức,
                        // nhả các chunk đang tải dở về lại Queue.
                        if (_pauseCts != null && !_pauseCts.IsCancellationRequested)
                        {
                            _pauseCts.Cancel();
                        }
