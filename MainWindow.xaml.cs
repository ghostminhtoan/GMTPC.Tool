...
namespace GMTPC.Tool
{
    public partial class MainWindow : Window
    {
        ...
        private void Window_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            ...
            // Xử lý các phím mới cho tab Runtime và Driver (theo số thứ tự)
            if (e.Key == System.Windows.Input.Key.D7) // Phím 7 - Chạy nút Vcredist 2005-2022
            {
                BtnVcredist_Click(sender, new RoutedEventArgs());
                e.Handled = true; // Đánh dấu sự kiện đã được xử lý
                return; // Thoát để không xử lý các case khác
            }
            if (e.Key == System.Windows.Input.Key.D8) // Phím 8 - Chạy nút Directx
            {
                BtnDirectX_Click(sender, new RoutedEventArgs());
                e.Handled = true; // Đánh dấu sự kiện đã được xử lý
                return; // Thoát để không xử lý các case khác
            }
            // Thêm xử lý phím tắt '9' cho Java
            if (e.Key == System.Windows.Input.Key.D9) // Phím 9 - Chạy nút Java
            {
                BtnJava_Click(sender, new RoutedEventArgs());
                e.Handled = true; // Đánh dấu sự kiện đã được xử lý
                return; // Thoát để không xử lý các case khác
            }
            // Thêm xử lý phím tắt '0' cho OpenAL (sử dụng D0 vì D10 không tồn tại)
            if (e.Key == System.Windows.Input.Key.D0 && Keyboard.IsKeyDown(Key.LeftShift)) // Phím Shift+0 (hoặc có thể dùng cách khác)
            {
                // Không sử dụng D0 cho OpenAL vì D0 đã dùng cho "Ủng hộ tác giả"
            }
            // Thay vào đó, sử dụng phím chữ cái cho OpenAL
            if (e.Key == System.Windows.Input.Key.A) // Phím A - Chạy nút OpenAL
            {
                BtnOpenAL_Click(sender, new RoutedEventArgs());
                e.Handled = true; // Đánh dấu sự kiện đã được xử lý
                return; // Thoát để không xử lý các case khác
            }
            // Thêm xử lý phím tắt 'c' cho 3DP Chip
            if (e.Key == System.Windows.Input.Key.C) // Phím C - Chạy nút 3DP Chip
            {
                Btn3DPChip_Click(sender, new RoutedEventArgs());
                e.Handled = true; // Đánh dấu sự kiện đã được xử lý
                return; // Thoát để không xử lý các case khác
            }
            // Thêm xử lý phím tắt 'n' cho 3DP Net
            if (e.Key == System.Windows.Input.Key.N) // Phím N - Chạy nút 3DP Net
            {
                Btn3DPNet_Click(sender, new RoutedEventArgs());
                e.Handled = true; // Đánh dấu sự kiện đã được xử lý
                return; // Thoát để không xử lý các case khác
            }

            // Xử lý các phím số cho tab Downloader tool (giữ nguyên phần này)
            switch (e.Key)
            {
                case System.Windows.Input.Key.D1: // Phím 1 - Cài đặt và kích hoạt IDM
                    BtnInstallIDM_Click(sender, new RoutedEventArgs());
                    break;
                case System.Windows.Input.Key.D2: // Phím 2 - Tự động kích hoạt Windows
                    BtnActivateWindows_Click(sender, new RoutedEventArgs());
                    break;
                case System.Windows.Input.Key.D3: // Phím 3 - Tự động kích hoạt Office
                    BtnActivateOffice_Click(sender, new RoutedEventArgs());
                    break;
                case System.Windows.Input.Key.D4: // Phím 4 - Pause Windows Update
                    BtnPauseWindowsUpdate_Click(sender, new RoutedEventArgs());
                    break;
                case System.Windows.Input.Key.D5: // Phím 5 - Cài và activate Winrar
                    BtnInstallWinRAR_Click(sender, new RoutedEventArgs());
                    break;
                case System.Windows.Input.Key.D6: // Phím 6 - Cài và kích hoạt Bulk Image Downloader
                    BtnInstallBID_Click(sender, new RoutedEventArgs());
                    break;
                case System.Windows.Input.Key.D0: // Phím 0 - Ủng hộ tác giả
                    BtnExit_Click(sender, new RoutedEventArgs());
                    break;
            }
        }
